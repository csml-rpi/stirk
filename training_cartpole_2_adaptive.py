...
for loop in range(num_loops):
    path = os.path.join(case.filepath, f'loop_{loop}')
    makedir(path)
    case.save_gt_info(filepath=path)
    case.save_observable_info(filepath=path)
    case.save_test_data(filepath=path)
...
for loop in range(num_loops):
    path = os.path.join(case.filepath, f'loop_{loop}')
    rollout_edmd.filepath = path
    rollout_edmd.set_optimizers(scheduler_mode=args.scheduler_mode, config_cyclic_lr=config_cyclic_LR)
    rollout_edmd.setup_comparison(x_true=x, x_noisy=x_noisy, x0_true=x0, steps=case.steps, u_true=u)
    A, B, error_dict = rollout_edmd.fit(x_noisy, u=u, strategy=case.strategy, start_roll_out_length=args.start_roll_out_length, start_epoch=1, num_epochs=args.num_epochs)
    koopman_model_mpc = LinearModelDiscrete(A=A, B=B)

    mpc_with_surrogate = MPC(model=model,surrogate_model=koopman_model_mpc,observable=observable)
    set_mpc(mpc_with_surrogate, Q, lower_u, upper_u, setup_mpc)
    def run_mpc_and_plots(x0, path):
        run_mpc(mpc_with_surrogate,steps, x0)
        sim_data = mpc_with_surrogate.simulator.data

        mpc_with_surrogate.save(f'training_data_{np.round(x0.numpy().squeeze(), decimals=2)}', steps, path=path)
        ...
        fig.savefig(f"{path}/mpc_results_training_data_{np.round(x0.numpy().squeeze(), decimals=2)}.png")
        return sim_data
    ...
    x0_mpc = torch.concat([position_0, theta_0, velocity_0, omega_0], dim=-1).to(device=x0.device, dtype=x0.dtype)

    x_opt = []
    u_opt = []
    for x_init in x0_mpc.cpu(): #x0.cpu():
        x_init.unsqueeze_(0)
        sim_data = run_mpc_and_plots(x_init, path=path)
        x_np = torch.from_numpy(sim_data["_x"])
        u_np = torch.from_numpy(sim_data["_u"])
        x_opt.append(x_np)
        u_opt.append(u_np)

    x_opt = torch.stack(x_opt, dim= 0).to(device=config.device, dtype=config.dtype)
    u_opt = torch.stack(u_opt, dim= 0)[:,:-1,:].to(device=config.device, dtype=config.dtype)
    x = torch.cat([x, x_opt], dim = 0)
    u = torch.cat([u, u_opt], dim=0)
    
    gaussian_noise = torch.normal(mean=0, std=1.0, size=x_opt.shape).to(device=x_opt.device, dtype = x_opt.dtype)
    scaled_noise = gaussian_noise * (x_opt.abs() * noise)
    x_opt = x_opt + scaled_noise

    x_noisy = torch.cat([x_noisy, x_opt], dim=0)
    x0 = torch.cat([x0, x0_mpc], dim=0)
    # print(x_opt.shape)
    # print(u_opt.shape)
    # print(x0.shape)

    trajectory_data = dict(x0=x0, x= x, u=u, x_noisy=x_noisy)
    save_file(trajectory_data, f"{path}/new_trajectories_data_loop{loop+1}.pkl")
